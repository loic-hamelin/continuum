...
import { ConflictResolutionPanel, buildResolutions } from "./ConflictResolutionPanel";
import { NodeGroupList, type NodeRow } from "./NodeGroupList";

export function ComparisonTab({ apiConnected }: { apiConnected: boolean }) {
  ...
  useEffect(() => {
    ...
    loadReferenceNodes();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiConnected, reference]);

  useEffect(() => {
    if (!apiConnected) return;
    loadDiff();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiConnected, reference, selectedBranch]);

  useEffect(() => {
    if (!apiConnected) return;
    loadHistory();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiConnected, selectedBranch]);
